/**
 * OAuthButtons — renders social login buttons fetched from the API.
 * Gracefully hides if no providers are configured.
 */
        ...
        {providers.map(p => {
          ...
          return (
            <a
              key={p.name}
              href={p.url}
              className="flex items-center justify-center gap-3 w-full px-4 py-2.5 rounded-xl border border-ink-200 bg-white hover:bg-ink-50 transition-colors text-sm font-medium text-ink-700"
            >
              <Icon />
              {p.label}
            </a>
